...
function StatsView({ dayInstances, completedInstances, commitmentTemplates }) {
  ...
  // Load gym and content data for cross-reference + backend stats
  useEffect(() => {
    loadStatsData();
  }, []);
  ...
  const calculateMetrics = () => {
    ...
    Object.keys(dayInstances).forEach((dayIdx) => {
      ...
      instances.forEach((inst) => {
        ...
        const title = (template?.name || inst.title || "").toLowerCase();
    ...
    return {
      focus:
        focusTotal > 0 ? Math.round((focusCompleted / focusTotal) * 100) : 0,
      quran:
        quranTotal > 0 ? Math.round((quranCompleted / quranTotal) * 100) : 0,
      training:
        trainingTotal > 0
  ...
  const ProgressBar = ({ label, percentage, colorClass }) => (
    <div>
      <div className="flex justify-between mb-2">
        <span className="text-slate-900 dark:text-slate-400 font-black text-sm uppercase tracking-tighter">
          {label}
        </span>
    ...
    <div className="pb-20 space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div className="glass-card p-6 border-l-4 border-orange-500">
          <div className="text-xs text-slate-900 dark:text-slate-400 mb-2 font-black uppercase tracking-widest">
            Current Streak
          </div>
          ...
          <p className="text-xs text-slate-700 dark:text-slate-500 mt-2 font-bold">
            ...
            {(() => {
              ...
              Object.keys(dayInstances).forEach((idx) => {
                ...
                if (insts && insts.length > 0) {
                  activeDays++;
                  const isDayComplete = insts.every(
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <section className="glass-card p-6">
          <h2 className="text-xl font-black mb-6 text-slate-900 dark:text-slate-100 uppercase tracking-tighter">
            Weekly Completion
          </h2>
          <div className="h-64 flex items-end justify-between gap-2">
            {DAYS.map((day, idx) => {
              ...
              return (
                ...
                >
                  <div
                    className="w-full bg-slate-700/50 dark:bg-slate-800/50 rounded-t-lg relative flex items-end justify-center overflow-hidden hover:bg-slate-600 transition"
                    style={{ height: "100%" }}
                  >
                    <div
                  ...
                  <span className="mt-2 text-xs font-black text-slate-900 dark:text-slate-400">
                    {day}
                  </span>
                  <div className="text-xs font-black text-slate-700 dark:text-slate-200 mt-1">
                    {percent}%
                  </div>
                </div>
              );
        ...
        {/* Detailed Metrics */}
        <section className="glass-card p-6">
          <h2 className="text-xl font-black mb-6 text-slate-900 dark:text-white uppercase tracking-tighter">
            Metric Adherence
          </h2>
          <div className="space-y-6">
            <ProgressBar
              label="Focus Block Adherence"
              percentage={metrics.focus}
              colorClass="text-amber-400"
            />
            <ProgressBar
              label="Quran Consistency"
              percentage={metrics.quran}
              colorClass="text-green-400"
            />
            <ProgressBar
              label="Training Completion"
              percentage={metrics.training}
