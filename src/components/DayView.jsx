...
function DayView({
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            {/* Modal Content */}
            <div className="relative bg-white dark:bg-[#0f172a] w-full max-w-4xl max-h-[90vh] rounded-3xl shadow-2xl overflow-hidden flex flex-col border border-slate-200 dark:border-slate-800">

                {/* Header */}
                <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/30">
                    <div>
                        <h2 className="text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tighter flex items-center gap-3">
                            <span className="text-blue-600 dark:text-blue-400">{dayName}</span>
                            <span className="text-slate-300 dark:text-slate-700">Detailed View</span>
                        </h2>
                        <p className="text-slate-500 dark:text-slate-400 mt-1 font-medium">Manage your daily priorities and notes.</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
                        ...
                        <div className="space-y-3">
                            {instances.length === 0 ? (
                                <div className="p-12 text-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl">
                                    ...
                                    <p className="text-slate-400 dark:text-slate-600 font-medium">No commitments for this day.</p>
                                </div>
                            ) : (
                                instances.map((instance) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex-1">
                                            <h4 className="font-bold text-slate-800 dark:text-slate-200">{instance.title}</h4>
                    ...
                    <div className="lg:col-span-5 space-y-8">
                        ...
                        <div className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-3xl border border-slate-200 dark:border-slate-800">
                            <h3 className="text-xs font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-4">Day Performance</h3>
                            <button
                                onClick={onToggleDayStatus}
                                className={`w-full p-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${isDayMarked
                                        ? "bg-green-500 text-white shadow-lg shadow-green-500/20"
                                        : "bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-blue-500 hover:text-white"
                                    }`}
                            >
                                <i className={`fa-solid ${isDayMarked ? 'fa-award' : 'fa-bullseye'} mr-2`}></i>
                        ...
                        <div className="space-y-4">
                            <h3 className="text-xs font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500 flex justify-between">
                                <span>Notes</span>
                                {noteText !== dayNote && <span className="text-blue-500 animate-pulse text-[10px]">Unsaved</span>}
                            </h3>
                            <textarea
                                value={noteText}
                                onChange={(e) => setNoteText(e.target.value)}
                                onBlur={() => onAddNote(dayIdx, noteText)}
                            ...
                            ></textarea>
                            <p className="text-[10px] text-slate-400 dark:text-slate-600 px-2 italic font-medium">Auto-saves when you click out of the box.</p>
                        </div>
